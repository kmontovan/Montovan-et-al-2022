#This r script was created by Katie Montovan hold the subfunctions
#It was last updates on 1/3/2021


#This function runs the model and returns the size of the prey when the predator visit
#starts and the probability the prey survives to the size of maturity if it a) hides 
# and b) forages during the predator visit. 'Choice' reports which behavior is optimal
# (leads to a higher survival probability).
...
# was PotatoBeetle_G
# For non cannibals, can_timeshift=1.
...
# Functions for making the plot of elasticities for the sensitivity analysis
LollyPopPlot <- function(y, names,ylimits,nvar) {
  par(mfrow=c(1,1),cex.axis=2,cex.lab=2)
  px=1:nvar;
  matplot(px,y,type="p",col=c("black"), pch=15, cex=2.5,cex.axis=1,cex.lab=1.4,
          xaxt="n",xlab="Parameters",ylab="Elasticity", ylim=ylimits,xlim=c(0.75,nvar+.25)); 
  axis(side=1,at=1:nvar,labels=names,cex.axis=1.2);
  polygon( x = c(0,0,1.5,1.5), y=c(ylimits[1]-10, ylimits[2]+10, ylimits[2]+10, ylimits[1]-10) ,col="gray90",border="gray")
  polygon( x = c(1.5,1.5,3.5,3.5), y=c(ylimits[1]-10, ylimits[2]+10, ylimits[2]+10, ylimits[1]-10) ,col="gray90",border="gray")
  polygon( x = c(3.5,3.5,5.5,5.5), y=c(ylimits[1]-10, ylimits[2]+10, ylimits[2]+10, ylimits[1]-10) ,col="gray90",border="gray")
  
  points(px,y,type="p",col=c("black"), pch=15,cex=2.5)
